...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  if model_architecture == 'single_fc':
    ...
    return create_conv_model(fingerprint_input, model_settings, is_training, is_transfer)
...
def create_conv_model(fingerprint_input, model_settings, is_training, is_transfer):
  ...
  if is_transfer:
      set_trainable=False
      import numpy as np
      container = np.load('models/pretrained_model/pretrained_numpy_weights_1400duration.npz')
      npweights = [container[key] for key in container]

      first_weights = tf.compat.v1.get_variable(
          name='first_weights',
          initializer=tf.compat.v1.constant(npweights[0]), trainable=set_trainable)
      first_bias = tf.compat.v1.get_variable(
          name='first_bias',
          initializer=tf.compat.v1.constant(npweights[1]), trainable=set_trainable)
      ...
      second_weights = tf.compat.v1.get_variable(
          name='second_weights',
          initializer=tf.compat.v1.constant(npweights[2]), trainable=set_trainable)
      second_bias = tf.compat.v1.get_variable(
          name='second_bias',
          initializer=tf.compat.v1.constant(npweights[3]), trainable=set_trainable)
      ...
      first_weights = tf.compat.v1.get_variable(
          ...
          shape=[first_filter_height, first_filter_width, 1, first_filter_count], trainable=set_trainable)
      first_bias = tf.compat.v1.get_variable(
          name='first_bias',
          initializer=tf.compat.v1.zeros_initializer,
          shape=[first_filter_count], trainable=set_trainable)
      first_conv = tf.nn.conv2d(input=fingerprint_4d,
                                filters=first_weights,
                                strides=[1, 1, 1, 1],
                                padding='SAME') + first_bias
